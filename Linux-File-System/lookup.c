 ...
 * Since space allocated, create inode connection.
 */
				      ...
				      struct path *lower_parent_path)
{
	...
	/* Use vfs_path_lookup to check if the dentry exists or not, struct is filled */
	...
	/* instatiate a new negative lower dentry */
	this.name = name;
			     ...
			     unsigned int flags)
{
	...
	/* Only allocate memory for dentry private data.  We free it in ->d_release */
...
out:
	bkpfs_put_lower_path(parent, &lower_parent_path); //release memory
	dput(parent);
	return ret;
}
