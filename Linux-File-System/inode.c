			 ...
			 umode_t mode, bool want_excl)
{
	...
	folder_name = kmalloc(strlen(BKP_FOLDER_PREFIX) + 1 + strlen(dentry->d_name.name) + 1, GFP_KERNEL);
	strcpy(folder_name,  BKP_FOLDER_PREFIX);
	strcat(folder_name, dentry->d_name.name);
	...
	dentry_t = d_lookup(lower_parent_dentry, &this);
	if (dentry_t) {
		UDBG;
		printk("bkp dir already exists. Something wrong");
		goto out;
	}

	dentry_t = d_alloc(lower_parent_dentry, &this);
	if (!dentry_t) {
		dput(dentry_t);
		err = -ENOMEM;
		UDBG;
	...
	d_add(dentry_t, NULL); /* instantiate and hash */

	err2 = vfs_path_lookup(lower_parent_dentry, lower_path.mnt, dentry_t->d_name.name, 0,
			      &lower_path_bkp_dir);
	if (!err2) {
		printk("bkp dir already exists. Something wrong");
		goto out;
	...
	if (err2 != -ENOENT) {
		printk("Unknown error. Something wrong");
		goto out;
	}

	printk("Tryng to create bkp dir");
	err2 = vfs_mkdir(d_inode(lower_parent_dentry), dentry_t, mode);

	if (err2) {
		printk("Failed to create bkp dir");
		goto out;
	}

	// spin_lock_init(&dentry_t->d_lock);
	...
	err2 = vfs_setxattr(dentry_t, BKP_ATTRIBUTE_KEY, dentry_meta_data, sizeof(struct lower_dentry_meta_data), 0);
	printk("return %d", err2);	
	if (!err2) {
